...
public final class SplunkSinkTask extends SinkTask implements PollerCallback {
  ...
  private Event createHecEventFrom(final SinkRecord record) {
    ...
    //add fields from message headers
    Map<String, String> fieldsFromHeaders = new HashMap<>();
    for (Header hdr : record.headers()) {
      if (hdr.key().startsWith(SplunkSinkConnectorConfig.FIELDS_HDR_PREFIX)) {
        String name = hdr.key().substring(SplunkSinkConnectorConfig.FIELDS_HDR_PREFIX.length());
        String value = hdr.value() == null ? "" : hdr.value().toString();
        fieldsFromHeaders.put(name, value);
      }
    }
    if (fieldsFromHeaders.size() > 0) {
      event.addFields(fieldsFromHeaders);
    }
